...
def single_seed_bulk_and_non_bulk_classification(
    ...
    sample_numbers: np.ndarray, sample_types: np.ndarray,
    who_grades: np.ndarray, spectras_bulk: np.ndarray,
  ...
  (
      ...
      sample_types_bulk.copy(), model_type, seed, None, None, n_trials, n_jobs
  )
...
def multiple_seeds_classification_with_parallel(
    ...
    processed_files: List[Path], metadata_df: pd.DataFrame, agg_func: str,
    output_dir: Path, n_trials: int = 50, n_jobs: int = -1
) -> List[int]:
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      '--agg_func', type=str, choices=['mean', 'max', 'median', 'min'],
      default='mean', help="Aggregation method ('mean', 'max', 'median', 'min')"
  )
  parser.add_argument(
  ...
  evaluation_seeds = multiple_seeds_classification_with_parallel(
      ...
      metadata_df, args.agg_func, output_path, n_trials=50, n_jobs=-1
  )
